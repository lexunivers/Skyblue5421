        ...
        if ($form->isSubmitted()) {
            ...
            if ($isFormValid && (!$this->isInPreviewMode($request) || $this->isPreviewApproved($request))) {
                ...
                //Ajout de code pour renseigner entity\Resources //
					...
					// On lie Avion à Resources
					$form->getData()->setAvion($resources);
                //Fin de l'Ajout//
    ...
    public function preEdit(Request $request, object $object): ?Response
    {
        ...
        if ($form->isSubmitted()) {
            ...
            if ($isFormValid && (!$this->isInPreviewMode($request) || $this->isPreviewApproved($request))) {
				...
				$form->getData()->getAvion()->setTitle($submittedObject->getImmatriculation());
				$form->getData()->getAvion()->setMarque($submittedObject->getMarque());
				$form->getData()->getAvion()->setType($submittedObject->getType());								
				$form->getData()->getAvion()->setEventColor($submittedObject->getEventColor());
